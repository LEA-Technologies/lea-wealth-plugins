...
async function graphRequest(method, path, body) {
  ...
  if (response.status === 401) {
    ...
    try {
      ...
      if (!retry.ok) {
        ...
        console.error(`SharePoint API error ${retry.status}:`, errBody);
        throw new Error(`SharePoint API request failed (${retry.status}). Check connection and retry.`);
      }
      return retry.json();
    } catch (err) {
  ...
  if (!response.ok) {
    ...
    console.error(`SharePoint API error ${response.status}:`, errBody);
    throw new Error(`SharePoint API request failed (${response.status}). Check connection and retry.`);
  }
