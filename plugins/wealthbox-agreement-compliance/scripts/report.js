/**
 * HTML report generator for Wealthbox Agreement Compliance.
...
function generateComplianceReport(data) {
  const {
    platform, totalWbHouseholds, totalWbContacts, totalVaultFolders,
    totalMatched, totalUnmatchedWb, totalUnmatchedVault,
    withAllAgreements, missingAny,
    matched, unmatchedWb, unmatchedVault
  } = data;

  const platformLabel = { box: 'Box', sharepoint: 'SharePoint', egnyte: 'Egnyte' }[platform] || platform;
  const matchRate = totalWbHouseholds > 0 ? Math.round((totalMatched / totalWbHouseholds) * 100) : 0;
...
<head>
  ...
  <title>Wealthbox Agreement Compliance Report</title>
...
<body class="bg-gradient-to-br from-slate-50 to-slate-100 min-h-screen">
  <div class="max-w-[1400px] mx-auto px-4 py-8">
        ...
        <div>
          <div class="flex items-center gap-4">
            ...
            <div class="border-l border-slate-200 pl-4">
              ...
              <p class="text-slate-500 text-sm">Wealthbox + ${escapeHtml(platformLabel)}</p>
            </div>
          </div>
        </div>
    ...
    <div class="grid grid-cols-4 gap-4 mb-6">
      <div class="bg-white rounded-xl shadow-sm border border-slate-200 p-5 text-center">
        <p class="text-sm text-slate-500 mb-1">WB Households</p>
        <p class="text-3xl font-bold text-slate-900">${totalWbHouseholds}</p>
        <p class="text-xs text-slate-400 mt-1">${totalWbContacts} contacts total</p>
      </div>
      <div class="bg-white rounded-xl shadow-sm border border-slate-200 p-5 text-center">
      ...
      <div class="bg-white rounded-xl shadow-sm border border-${matchRate >= 80 ? 'emerald' : matchRate >= 50 ? 'amber' : 'red'}-200 p-5 text-center ${matchRate >= 80 ? 'bg-emerald-50' : matchRate >= 50 ? 'bg-amber-50' : 'bg-red-50'}">
        ...
        <p class="text-xs text-slate-400 mt-1">${totalMatched} of ${totalWbHouseholds} matched</p>
      </div>
      ...
      <div class="p-5 border-b border-slate-200">
        <div class="flex justify-between items-center">
          <div>
            ...
            <p class="text-slate-400 text-xs mt-1">${totalMatched} households matched between Wealthbox and ${platformLabel}</p>
          </div>
    ...
    <!-- Unmatched WB Households -->
    <div class="bg-white rounded-xl shadow-sm border border-amber-200 mb-6">
      <div class="p-5 border-b border-amber-200 bg-amber-50">
        <h2 class="text-lg font-semibold text-amber-800">Wealthbox Households Without Vault Match</h2>
        <p class="text-amber-600 text-xs mt-1">${unmatchedWb.length} households in Wealthbox have no matching folder in ${platformLabel}</p>
      </div>
      <div class="p-5">
    ...
    <div class="bg-white rounded-xl shadow-sm border border-slate-200 mb-6">
      <div class="p-5 border-b border-slate-200">
        <h2 class="text-lg font-semibold text-slate-800">Vault Folders Without Wealthbox Match</h2>
        <p class="text-slate-400 text-xs mt-1">${unmatchedVault.length} folders in ${platformLabel} have no matching Wealthbox household</p>
      </div>
      <div class="p-5">
        <div class="grid grid-cols-3 gap-2">
  ...
  <script>
    ...
    const unmatchedWbData = ${JSON.stringify(unmatchedWb).replace(/<\//g, '<\\/')};
    ...
    function exportCSV() {
      ...
      for (const u of unmatchedWbData) {
      ...
      a.download = 'wealthbox-agreement-compliance.csv';
      a.click();
    }
  </script>
</body>
