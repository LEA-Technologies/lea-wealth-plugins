...
/**
 * Egnyte Client Agreement Organizer CLI — JSON-outputting command dispatcher
 *
 * Usage:
 ...
 *   node cli.js scan-agreements <path>
...
async function cmdScanAgreements(folderPath) {
  if (!folderPath) fail('Usage: cli.js scan-agreements <path> [--report <path>]');
  ...
  try {
    console.log(`Scanning ${folderPath} for client agreements...`);
    const households = await eg.scanAgreements(folderPath);
...
async function cmdOrganizeAgreements(jsonStr) {
  if (!jsonStr) fail('Usage: cli.js organize-agreements \'{"target_path": "...", "agreements": [...]}\'');
  ...
  const { target_path, agreements } = plan;
  if (!agreements || !Array.isArray(agreements)) {
    fail('JSON must include agreements array');
  }

  try {
    // Create target folder if specified
    let targetDir = target_path;
    if (plan.create_target_folder && plan.target_path) {
      console.log(`Creating folder: ${plan.target_path}...`);
      try {
        await eg.createFolder(plan.target_path);
      } catch (err) {
        // Folder may already exist — continue
        if (!err.message.includes('403') && !err.message.includes('Folder already exists')) {
          throw err;
        }
      }
    }
    ...
    for (const agreement of agreements) {
      try {
        const destPath = `${targetDir}/${agreement.new_name}`;
        console.log(`  Copying: ${agreement.original_name} → ${agreement.new_name}`);
        await eg.copyFile(agreement.src_path, destPath);
        results.push({
          src_path: agreement.src_path,
...
if (!command || !commands[command]) {
  console.log('Egnyte Client Agreement Organizer CLI');
  console.log('');
  ...
  console.log('  scan-agreements <path>                   Scan for client agreements');
