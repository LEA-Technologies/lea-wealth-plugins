/**
 * HTML report generator for Practifi Data Completeness Audit.
 * LEA-branded with Tailwind CDN, matching Wealthbox/Box/Egnyte/SharePoint report style.
 */
...
function generateAuditReport(auditData, firmName) {
  const { totalContacts, fieldsChecked, skippedFields, averageScore, needsAttention, fieldStats, contacts } = auditData;
  ...
  const groupOrder = ['Contact Basics', 'Personal', 'Compliance', 'Relationship', 'Financial'];
  ...
  const hasSkippedFields = skippedFields && skippedFields.length > 0;
...
<head>
  ...
  <title>Practifi Data Completeness Report${firmName ? ' - ' + escapeHtml(firmName) : ''}</title>
...
<body class="bg-gradient-to-br from-slate-50 to-slate-100 min-h-screen">
  <div class="max-w-[1200px] mx-auto px-4 py-8">
        ...
        <div>
          <div class="flex items-center gap-4">
            ...
            <div class="border-l border-slate-200 pl-4">
              <h1 class="text-2xl font-bold text-slate-900">Practifi Data Completeness Audit <span class="text-base font-normal text-slate-400">- a LEA Claude Skill</span></h1>
    ...
    <div class="grid grid-cols-4 gap-4 mb-6">
      ...
      <div class="bg-white rounded-xl shadow-sm border border-slate-200 p-5 text-center">
        <p class="text-sm text-slate-500 mb-1">Fields Checked</p>
        <p class="text-3xl font-bold text-slate-900">${fieldsChecked}</p>
        ${hasSkippedFields ? `<p class="text-xs text-slate-400 mt-1">${skippedFields.length} field${skippedFields.length > 1 ? 's' : ''} not found in org</p>` : ''}
      </div>
      <div class="bg-white rounded-xl shadow-sm border border-slate-200 p-5 text-center">
        ...
        <p class="text-xs text-slate-400 mt-1">weighted score</p>
      </div>
    ...
    <div class="bg-amber-50 border border-amber-200 rounded-xl p-4 mb-6">
      <p class="text-sm text-amber-800"><strong>Note:</strong> The following Practifi custom fields were not found in your org and were excluded from the audit: <em>${skippedFields.join(', ')}</em>. These may not be configured in your Practifi setup.</p>
    </div>
    ` : ''}
  ...
  <script>
    ...
    function exportCSV() {
      const headers = ['Name', 'Email', 'Phone', 'Date of Birth', 'Household', 'Assets', 'Score', 'Missing Fields Count', 'Missing Fields'];
      const rows = [headers];
      ...
      a.download = 'practifi-data-completeness-audit.csv';
      a.click();
    }
  </script>
</body>
